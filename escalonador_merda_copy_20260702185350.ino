...
void interpolar(float origem[], int qtdOrigem, float destino[])
{
    for (int i = 0; i < 100; i++)
    {
        float pos = (float)i * (qtdOrigem - 1) / 99.0;
        ...
        if (j >= qtdOrigem - 1)
        {
            destino[i] = origem[qtdOrigem - 1];
        }
        else
        {
            destino[i] =
                origem[j] +
                frac * (origem[j + 1] - origem[j]);
        }
    }
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (texto.startsWith("sinal ")) {
      ...
      if (funcao == "Personalizado"){
        float ponto1 = listaValores[0];
        for(int i = 100; ponto1 != listaValores[i]; i--){
          qtdValores--;
          if(i == 0){
            qtdValores = 100;
          }
