...
void printobj();
float calculaAmplitude(Opcao op);
float calculaFrequencia(Opcao op);
...
void redesenhaCurva(uint16_t cor)
{
    ...
    for (int px = G_X0 + 1; px <= G_X1; px++)
    {
       ...
       // py = constrain(py, G_Y0, G_Y1 - 1);
        if (pxAnt != G_X0+1)
            lcd.drawLine(pxAnt, pyAnt, px, py, cor);
        else
...
void atualizaInfoEncoders()
{
    ...
    lcd.print("amplitude: ");
    lcd.print((float)op.amp);
    lcd.setCursor(G_X1 - 110, G_Y1 + 4);
    lcd.print("frequencia: ");
    lcd.print((float)op.freq);
}
...
float calculaAmplitude(Opcao op) {
  maximo = op.sinal.valores[0];
  minimo = op.sinal.valores[0];

  for (int i = 1; i < qtdValores; i++) {
    if (op.sinal.valores[i] > maximo) maximo = op.sinal.valores[i];
    if (op.sinal.valores[i] < minimo) minimo = op.sinal.valores[i];
  }

  float amplitudeAtual = (maximo - minimo) / 2;
...
float calculaFrequencia(Opcao op) {
  ...
  int ciclos = 0;
  float ref = op.sinal.valores[0];

  for (int i = 1; i < 100; i++) {
    if (op.sinal.valores[i] == ref) {
      ciclos++;
    }
  }
  float frequenciaAtual = ciclos;
...
void atualizaDAC()
{
    ...
    if (op.sinal.qtdPontos <= 0) return;

    
    unsigned long tempoEntrePontos = 1000000 / (op.freq * op.sinal.qtdPontos);

    if (micros() > instanteAnteriorDeDeteccao + tempoEntrePontos)
    {
        instanteAnteriorDeDeteccao = micros();

        if (indiceDAC >= op.sinal.qtdPontos)
        {
            indiceDAC = 0;
        }
        ...
        valorTabela = valorTabela * (op.amp / 50.0f);

        int valorDAC = map(valorTabela, -100, 100, 0, 4095);
...
{   
    ...
    if (texto.startsWith("sinal ")) {
      ...
      for (int i = 0; i < qtdValores; i++) {
        float valor = (listaValores[i]*100.0f);
        valor = constrain(valor,-100.0f,100.0f);
        op.sinal.valores[i]=valor;
      }
      op.cor = COR_COS;
      op.freq = constrain(calculaFrequencia(op), 0.1, 1);
      op.amp = constrain(calculaAmplitude(op), 1, 50);
